						...
						success : function (aEntitySet) {
							...
							return this.waitFor({
								...
								check: function (oList) {
									...
									var sObjectID = oList.getBindingContext().getProperty("Matnr");

									var iLength = aEntitySet.filter(function (oLineItem) {
										return oLineItem.Matnr === sObjectID;
									}).length;
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							return this.waitFor({
								id : "lineItemsHeader",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name: "text", value: "<MaterialPlural> (" + iNumberOfItems + ")"}),
								success : function () {
